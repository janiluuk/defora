...
python -m defora_cli.forge_cli "a synthwave city at night, neon, wide shot"

# 2) Explicit img subcommand with some overrides
python -m defora_cli.forge_cli img -W 832 -H 1216 -n 2 \
  "moody portrait, cinematic lighting"

# 3) See which models are installed and their detected profiles
python -m defora_cli.forge_cli models

# 4) Switch to a specific model by substring (see `models` output)
python -m defora_cli.forge_cli --model "revAnimated" img "anime girl in a city"
...
python -m defora_cli.forge_cli deforum --preset my_deforum_preset.json \
...
Examples:
  ...
  python -m defora_cli.forge_cli "a synthwave city at night"
      Quick txt2img with auto model choice (prefers Flux1-schnell).

  python -m defora_cli.forge_cli --model "flux" img -n 4 "dreamy portrait, cinematic"
  ...
  python -m defora_cli.forge_cli deforum --preset preset.json "dreamlike forest"
