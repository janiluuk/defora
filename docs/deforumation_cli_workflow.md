...
- With prompts only:
  - `./deforumation_request_dispatcher --prompt "a city at night" --negative "text, watermark" --frame-count 240 --steps 18 --strength 0.6 --cfg 6.0 --execute`
- With a Deforum JSON preset:
  - `./deforumation_request_dispatcher --preset my_preset.json --execute`
...
### Stream the render
- `./stream_helper start --source runs/<id>/frames --target rtmp://…/key --fps 24`
- `./stream_helper status` / `stop`
