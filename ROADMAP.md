...
| **Multi-band / “spectral” audio** (marketing language) | Named Hz presets (`bass_mid_high` CLI layout + Web band chips); envelope follower + `--smooth` on curves; **offline spectrogram** on Web upload (MODULATION + LIVE strip) | **Done (MVP)** · Advanced Audio for live AnalyserNode / richer viz |
...
- ✅ **MOTION sequencer**: keyframed `liveParam` tracks, per-segment easing, save/load via API, export JSON
...
- ✅ Audio waveform visualization (LIVE strip) and **upload spectrogram** (FFT overview in MODULATION)
...
- **Features**:
  ...
  - **Spectral visualization**: ✅ offline FFT spectrogram after Web upload (Phase 7); optional future: live `AnalyserNode` while playing
...
#### Animation Sequencer
- **Priority**: High
- **MVP (Phases 2 + 6)**: ✅ Keyframed tracks with optional per-segment cubic easing (`linear` / `easeIn` / `easeOut` / `easeInOut`), server persistence (`GET/POST/DELETE /api/sequencer`), MOTION tab playback → mediator via `liveParam`, JSON export — see `docs/API.md` / `SEQUENCER_DIR`.
- **Remaining (polish)**:
