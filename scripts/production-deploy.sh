...
echo "  Mediator: ${DEF_MEDIATOR_HOST:-${MEDIATOR_HOST:-vimage2}}:${DEF_MEDIATOR_PORT:-${MEDIATOR_PORT:-8766}}"
echo "  Media (in web): /data/runs  (uploads: /data/runs/uploads)"
echo "  Media (host): $(ssh -o StrictHostKeyChecking=accept-new -J \"$SSH_PROXY_JUMP\" \"${DEPLOY_USER}@${DEPLOY_HOST}\" \"docker volume inspect -f '{{.Mountpoint}}' defora_runs 2>/dev/null || echo defora_runs\" 2>/dev/null || echo defora_runs)"
