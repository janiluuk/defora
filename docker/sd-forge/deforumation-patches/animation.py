...
def anim_frame_warp_3d(device, prev_img_cv2, depth, anim_args, keys, frame_idx, shaker=None):
    ...
    def _maybe_shake(series, transform_type, axis, isDeforumation=False):
        if isDeforumation:
            if is_shake:
                return series + shaker.get_data(transform_type, axis, frame_idx)
            return series
        else:
            if is_shake:
    ...
    if usingDeforumation: #Should we Connect to the Deforumation websocket server to get translation values?
            ...
            if use_parseq_through_deforumator and int(mediator_getValue("should_use_deforumation_zoom")) == 1:
                deforumation_translation_z = float(mediator_getValue("translation_z").strip().strip('\n'))
                tzs = deforumation_translation_z                
            elif int(mediator_getValue("should_use_deforumation_zoom")) == 1:
                deforumation_translation_z = float(mediator_getValue("translation_z").strip().strip('\n'))
            ...
            translate_xyz = [
                #-txs * TRANSLATION_SCALE, 
                #tys * TRANSLATION_SCALE, 
                #-tzs * TRANSLATION_SCALE                
                _maybe_shake(txs, 'translation', 'x', True) * translation_scale * -1.0,
                _maybe_shake(tys, 'translation', 'y', True) * translation_scale,
                _maybe_shake(tzs, 'translation', 'z', True) * translation_scale * -1.0
            ]
    ...
    if usingDeforumation: #Should we Connect to the Deforumation websocket server to get rotation values?
            if use_parseq_through_deforumator and int(mediator_getValue("should_use_deforumation_rotation")) == 1:
                ...
                rys = keys.rotation_3d_y_series[frame_idx] + deforumation_rotation_y
            else:
            ...
            if use_parseq_through_deforumator and int(mediator_getValue("should_use_deforumation_tilt")) == 1:
                ...
                rzs = keys.rotation_3d_z_series[frame_idx]

            rotate_xyz = [
                #math.radians(rxs),
                #math.radians(rys),
                #math.radians(rzs)
                math.radians(_maybe_shake(rxs, 'rotation_3d', 'x', True)),
                math.radians(_maybe_shake(rys, 'rotation_3d', 'y', True)),
                math.radians(_maybe_shake(rzs, 'rotation_3d', 'z', True))
            ]
    ...
    #result = transform_image_3d_switcher(device if not device.type.startswith('mps') else torch.device('cpu'), prev_img_cv2, depth, rot_mat, 
    #translate_xyz, anim_args, keys, frame_idx)
...
def transform_image_3d_legacy(device, prev_img_cv2, depth_tensor, rot_mat, translate, anim_args, keys, frame_idx):
    ...
    use_parseq_through_deforumator = int(mediator_getValue("use_parseq").strip().strip('\n'))
    if usingDeforumation: #Should we Connect to the Deforumation websocket server to get rotation values?
        if int(mediator_getValue("should_use_deforumation_fov")) == 1:
