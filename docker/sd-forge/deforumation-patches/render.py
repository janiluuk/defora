...
from .animation_key_frames import DeformAnimKeys, LooperAnimKeys, ControlNetKeys, FreeUAnimKeys, KohyaHRFixAnimKeys
...
import json
import types, copy
from .deforum_mediator import mediator_getValue, mediator_setValue, mediator_set_anim_args, mediator_wasInterrupted
...
def render_animation(args, anim_args, video_args, parseq_args, loop_args, controlnet_args, freeu_args, kohya_hrfix_args, root):
    ...
    #End settings
    #Because DeforumationQT does not handle experimental core yet, we just ignore it (rem it out)
    ...
    if usingDeforumation:
        log_utils.info(f"We are using Deforumation")
    else:
        log_utils.info(f"We are NOT!!!!!!!!!! using Deforumation")
    if usingDeforumation:
        print("Made for DeforumationQT version: 0.1.9")
        print("------------------------------------")
    ...
    mediator_setValue("total_generated_images", start_frame)

    if usingDeforumation: #Should we Connect to the Deforumation websocket server to write the current resume frame properties?
        mediator_setValue("total_generated_images", 0)
        mediator_setValue("total_recall_relive", frame_idx)
    ...
    while frame_idx < anim_args.max_frames:
        ...
        if usingDeforumation: #Should we Connect to the Deforumation websocket server and get is_paused_rendering?
            ...
            if ispaused:
                print("** RESUMING **")
                if not mediator_wasInterrupted():
                    start_frame = int(mediator_getValue("start_frame").strip().strip('\n'))
                    print("The mediator was not interrupted, so resuming from start frame gotten from mediator:" + str(start_frame) + " --- but frame_idx is:" + str(frame_idx) + " .... and turbosteps are:" + str(turbo_steps))
                else:                    
                    start_frame = (frame_idx-turbo_steps)
                    print("Turbosteps were:" + str(mediator_getValue("cadence")))
                    cadence = turbo_steps
                    print("Setting Old cadence (turgbo steps)" + str(cadence))
                    mediator_setValue("cadence", cadence)
                    #print("The mediator was interrupted, so resuming from stored start frame (also setting mediator start frame):" + str(start_frame))
                    print("The mediator was interrupted, so resuming from start frame gotten from mediator:" + str(start_frame) + " --- but frame_idx is:" + str(frame_idx) + " .... and turbosteps are:" + str(turbo_steps))
                ...
                should_loop_back = int(mediator_getValue("render_loopback").strip().strip('\n'))
                if should_loop_back == 1:
                    frame_idx = start_frame + 1
                    start_frame = start_frame - turbo_steps
                    print("Is in loopback, and have:")
                    print("New StartFrame:"+str(start_frame))                    
                    print("What it thinks cadence is:" + str(frame_idx-start_frame-1))
                    print("Previous cadence is:" + str(turbo_steps))
                    #previous_turbo_steps = turbo_steps
                    if int(mediator_getValue("should_use_deforumation_cadence").strip().strip('\n')) == 1:
                        turbo_steps = int(mediator_getValue("cadence").strip().strip('\n'))
                    print("Current cadence is:" + str(turbo_steps))
                    previous_turbo_steps = turbo_steps
                    frame_idx = start_frame + turbo_steps + 1
                    print("New FrameIDX:"+str(frame_idx))
                    #if (frame_idx-start_frame-1) != turbo_steps:
                    #    start_frame = frame_idx - previous_turbo_steps -1
                    #    frame_idx = start_frame + turbo_steps + 1
                    #    print("!Gap is incorrect! Correcting start frame to:" + str(start_frame))
                    #else:
                    #    print("Gap is correct!.")

                    #if (frame_idx-start_frame-1) != turbo_steps:
                    #    start_frame = frame_idx - turbo_steps - 1
                    #    print("Gap is incorrect! Correcting start frame to:" + str(start_frame))
                    #else:
                    #    print("Gap is correct!")
            ...
            if (previous_turbo_steps != turbo_steps) or (shouldResume == 1):
                ...
                if frame_idx != start_frame or previous_turbo_steps != turbo_steps:
                    #First take care of special case (loopback function)
                    should_loop_back = int(mediator_getValue("render_loopback").strip().strip('\n'))
                    if should_loop_back == 1:
                        if was_in_paused_mode == 0: #Only triggers when loopback is on and cadence has been changed while not in paused mode
                            print("!!Is in loopback, and have:")
                            print("StartFrame:"+str(start_frame))
                            print("FrameIDX:"+str(frame_idx))
                            print("What it thinks cadence is:" + str(frame_idx-start_frame-1))
                            print("Actual new cadence is:" + str(turbo_steps))
                            print("Actual previous cadence was:" + str(previous_turbo_steps))
                            if (frame_idx-start_frame-1) != previous_turbo_steps:
                                start_frame = frame_idx - previous_turbo_steps -1
                                frame_idx = start_frame + turbo_steps + 1
                                print("Gap is incorrect! Correcting start frame to:" + str(start_frame))
                            else:
        ...
        if usingDeforumation:
            if (int(mediator_getValue("should_use_deforumation_cfg").strip().strip('\n')) == 1):
                cfg_scale = float(mediator_getValue("cfg").strip().strip('\n'))
                mediator_setValue("deforum_cfg", cfg_scale)
            else:
                cfg_scale = keys.cfg_scale_schedule_series[frame_idx]
                mediator_setValue("deforum_cfg", cfg_scale)
                mediator_setValue("cfg", cfg_scale)
        ...
        if turbo_steps > 1:
            ...
            should_loop_back = int(mediator_getValue("render_loopback").strip().strip('\n'))
            if should_loop_back == 0:
                frame_idx += turbo_steps
            else:
                print("!IN LOOPBACK MODE!!")         
        else:
        ...
        if usingDeforumation:        
            if int(mediator_getValue("is_inside_total_recall_range").strip().strip('\n')):
                ...
                seed_behavior = str(mediator_getValue("seed_behavior").strip().strip('\n'))
                args.seed_behavior = seed_behavior
                #print("Seed behavior is:" + str(seed_behavior))
                if args.seed_behavior == 'schedule' or parseq_adapter.manages_seed():
                    #print("Using scheduled seed, so no randomizing the seed.")
                    #args.seed = next_seed(args, root)
                    pass
                else:
                    args.seed_iter_N = int(mediator_getValue("seed_iter_n").strip().strip('\n'))
                    args.seed = next_seed(args, root)
