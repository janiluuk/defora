...
function ensureComponentStub(name, relPath, lines, seen = new Set()) {
  ...
  if (scriptMatch) {
    ...
    while ((m = importRe.exec(scriptMatch[1])) !== null) {
      ...
      if (nestedRel.includes('/views/') || ['RunsBrowserPanel.vue', 'FrameRailPanel.vue', 'SequencerControlsPanel.vue', 'StylesSettingsPanel.vue', 'VideoSwarmBrowser.vue', 'AnimationEnginePanel.vue', 'LiveEngineControls.vue', 'LiveEngineControlsDock.vue', 'CrossfaderPanel.vue', 'DeforumJobPanel.vue'].some((p) => nestedRel.endsWith(p))) {
...
script = script.replace(
  ...
  (_, name, relPath) => {
    if (relPath.includes('/views/') || ['RunsBrowserPanel.vue', 'FrameRailPanel.vue', 'SequencerControlsPanel.vue', 'StylesSettingsPanel.vue', 'VideoSwarmBrowser.vue', 'AnimationEnginePanel.vue', 'LiveEngineControlsDock.vue', 'CrossfaderPanel.vue', 'DeforumJobPanel.vue'].some((p) => relPath.endsWith(p))) {
