...
test('normalizeForgeProgressImage adds data url prefix', async () => {
  const { normalizeForgeProgressImage } = await loadEsm('..', 'src', 'shared', 'forge-preview-progress.mjs');
...
test('computeForgePreviewProgress prefers forge progress for single-frame preview', async () => {
  const { computeForgePreviewProgress } = await loadEsm('..', 'src', 'shared', 'forge-preview-progress.mjs');
...
test('computeForgePreviewProgress falls back to sampling steps', async () => {
  const { computeForgePreviewProgress } = await loadEsm('..', 'src', 'shared', 'forge-preview-progress.mjs');
  const result = computeForgePreviewProgress(
    {
      progress: 0,
...
test('computeForgePreviewProgress blends batch and forge for multi-frame jobs', async () => {
  const { computeForgePreviewProgress } = await loadEsm('..', 'src', 'shared', 'forge-preview-progress.mjs');
  const result = computeForgePreviewProgress(
