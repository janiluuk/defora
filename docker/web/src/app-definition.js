...
// --- inlined from utils/morph-utils.mjs (ESM source; do not edit) ---
...
// --- inlined from utils/deforum-settings-schema.mjs (ESM source; do not edit) ---
/**
...
// --- inlined from utils/deforum-settings-verify.mjs (ESM source; do not edit) ---
/**
...
// --- inlined from utils/api-utils.js (ESM source; do not edit) ---
/**
...
const STYLE_PREVIEW_BASE_PROMPT = 'bunny and cat in space';

/** Base scene for style preview generation (style.previewPrompt overrides default). */
function stylePreviewPromptFor(style) {
...
function commonVisualModKey(pluginId, paramId) {
  return `${pluginId}.common.${paramId}`;
}

function parseCommonVisualModKey(key) {
  const m = String(key || '').match(/^([a-z]+)\.common\.([a-z_]+)$/);
  if (!m) return null;
  return { pluginId: m[1], paramId: m[2] };
}

function bindingFor(pluginId, paramId) {
  return COMMON_VISUAL_BINDINGS[pluginId]?.[paramId] || { type: 'disabled' };
}

function isCommonVisualEnabled(pluginId, paramId) {
  const b = bindingFor(pluginId, paramId);
  return b.type !== 'disabled';
}

function buildCommonVisualModulationTargets(pluginId, pluginLabel) {
...
const GlassPanel = { props: ['size'], template: '<div class="glass-panel"><slot name="header"></slot><slot></slot></div>' };
