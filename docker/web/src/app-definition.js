...
const StatusStrip = { template: '<div></div>' };
...
module.exports = {
  ...
  data() {
    return {
      ...
      startupSequencerBootstrapped: false,
      startupSequencerBootstrapping: false,
  ...
  computed: {
    ...
    liveModulating() {
      ...
      this.lfoTargets.forEach((t) => {
        if (!paramMap[t.key]) {
          paramMap[t.key] = { key: t.key, label: t.label, val: t.default || 0, min: t.min || 0, max: t.max || 1 };
        }
      });
      const entries = [];
      const pushRoute = (route) => {
        const p = paramMap[route.key] || { key: route.key, label: route.key, val: 0, min: 0, max: 1 };
        entries.push({
          routeId: route.routeId,
          key: route.key,
          label: route.label || p.label,
          val: p.val,
          min: p.min,
          max: p.max,
          source: route.source,
          kind: route.kind,
          lfoId: route.lfoId,
          macroId: route.macroId,
          mapIndex: route.mapIndex,
          slotId: route.slotId,
        });
      };
      this.lfos.filter((l) => l.on && l.targets.length).forEach((lfo) => {
        lfo.targets.forEach((key) => {
          pushRoute({
            routeId: `lfo-${lfo.id}-${key}`,
            key,
            source: `LFO ${lfo.id}`,
            kind: 'lfo',
            lfoId: lfo.id,
      ...
      this.macrosRack.filter((macro) => macro.on && macro.target).forEach((macro) => {
        pushRoute({
          routeId: `macro-${macro.id}-${macro.target}`,
          key: macro.target,
          source: 'Macro',
          kind: 'macro',
          macroId: macro.id,
      ...
      if (this.audioReactiveActive) {
        this.audioMappings.forEach((mapping, mapIndex) => {
          if (!mapping || !mapping.param) return;
          pushRoute({
            routeId: `audio-${mapIndex}-${mapping.param}`,
            key: mapping.param,
            source: 'Audio',
            kind: 'audio',
            mapIndex,
      ...
      if (this.prompts.morphOn) {
        this.morphSlots.filter((slot) => slot.on).forEach((slot) => {
          entries.push({
            routeId: `morph-${slot.id}`,
            key: `morph-slot-${slot.id}`,
            label: slot.name || `Morph ${slot.id}`,
            val: Number(this.prompts.morphBlend) || 0,
            min: 0,
            max: 1,
  ...
  mounted() {
    ...
    deforumSettingsPromise.finally(() => {
      if (!this.deforumPlaying && !this.startupSequencerBootstrapping && this.startupSequencerBootstrapped) {
        this.schedulePreviewFrame();
      }
    });
    Promise.allSettled([deforumSettingsPromise, forgeRefreshPromise]).then(() => {
      this.restoreLastModel();
      void this.bootstrapStartupSequencerClip();
    });
    this.scanMidi();
 ...
 switchTab(id) {
   if (id === 'STREAM') {
     if (!this.streamDashboardOpen) this.toggleStreamDashboard();
     return;
   }
   if (id === 'AUDIO') {
 ...
 toggleStreamDashboard() {
   this.streamDashboardOpen = !this.streamDashboardOpen;
   if (this.streamDashboardOpen) void this.refreshStreamStatus();
 },
...
toggleCollaboration() {
 ...
 async initSystemFilesBrowser() {
   if (this.systemFiles.roots.length && this.systemFiles.currentPath) return;
   try {
     const { data } = await apiFetch('/api/system/files/roots', {}, 'system files roots');
     this.systemFiles.roots = Array.isArray(data.roots) ? data.roots : [];
     const first = this.systemFiles.roots[0];
     if (first) {
       this.systemFiles.rootId = first.id;
       await this.browseSystemFiles(first.path, { rootId: first.id });
     }
   } catch (err) {
     this.systemFiles.status = err.message || 'Could not load file roots';
   }
 },
 async browseSystemFiles(dirPath, opts = {}) {
   if (!dirPath) return;
   this.systemFiles.loading = true;
   this.systemFiles.status = '';
   try {
     const q = new URLSearchParams({
       path: dirPath,
       recursive: this.systemFiles.recursive ? 'true' : 'false',
       sort: this.systemFiles.sortKey,
     });
     const { data } = await apiFetch(`/api/system/files/browse?${q}`, {}, 'system files browse');
     this.systemFiles.currentPath = data.path || dirPath;
     this.systemFiles.parent = data.parent || null;
     this.systemFiles.videos = Array.isArray(data.videos) ? data.videos : [];
     this.systemFiles.videoCount = data.count ?? this.systemFiles.videos.length;
     if (opts.rootId) this.systemFiles.rootId = opts.rootId;
     this.systemFiles.selectedPaths = [];
     this.systemFiles.fullscreenIndex = -1;
   } catch (err) {
     this.systemFiles.status = err.message || 'Browse failed';
     this.systemFiles.videos = [];
     this.systemFiles.videoCount = 0;
   } finally {
     this.systemFiles.loading = false;
 ...
 refreshSystemFilesBrowse() {
   if (this.systemFiles.currentPath) {
     void this.browseSystemFiles(this.systemFiles.currentPath);
   } else {
     void this.initSystemFilesBrowser();
   }
 ...
 systemFileMediaUrl(filePath) {
   return `/api/system/files/media?path=${encodeURIComponent(filePath)}`;
 },
 openSystemFileFullscreen(index) {
   const n = Number(index);
   if (!Number.isFinite(n) || n < 0 || n >= this.systemFiles.videos.length) return;
   this.systemFiles.fullscreenIndex = n;
 },
 closeSystemFileFullscreen() {
   this.systemFiles.fullscreenIndex = -1;
 },
 stepSystemFileFullscreen(delta) {
   const list = this.systemFiles.videos || [];
   if (!list.length) return;
   let next = this.systemFiles.fullscreenIndex + delta;
   if (next < 0) next = list.length - 1;
   if (next >= list.length) next = 0;
   this.systemFiles.fullscreenIndex = next;
 },
 toggleSystemFileSelection(filePath) {
   const set = new Set(this.systemFiles.selectedPaths);
   if (set.has(filePath)) set.delete(filePath);
   else set.add(filePath);
   this.systemFiles.selectedPaths = [...set];
 },
 async copySystemFilePath(filePath) {
   if (!filePath) return;
   try {
     await navigator.clipboard.writeText(filePath);
     this.systemFiles.status = 'Path copied to clipboard';
   } catch (_e) {
     this.systemFiles.status = filePath;
   }
 },
 async deleteSystemFile(filePath) {
   if (!filePath) return;
   ...
   try {
     const q = new URLSearchParams({ path: filePath });
     await apiFetch(`/api/system/files?${q}`, { method: 'DELETE' }, 'system files delete');
     this.systemFiles.status = 'Deleted';
     if (this.systemFiles.fullscreenIndex >= 0) this.closeSystemFileFullscreen();
     this.refreshSystemFilesBrowse();
   } catch (err) {
     this.systemFiles.status = err.message || 'Delete failed';
   }
 },
...
toggleLfoTarget(lfo, targetKey) {
 ...
 removeActiveModRoute(route) {
   if (!route || !route.kind) return;
   switch (route.kind) {
     case 'lfo': {
       const lfo = this.lfos.find((item) => item.id === route.lfoId);
       if (lfo) this.toggleLfoTarget(lfo, route.key);
       break;
     }
     case 'macro': {
       const macro = this.macrosRack.find((item) => item.id === route.macroId);
       if (macro) {
         macro.on = false;
         macro.target = '';
       }
     ...
     case 'audio':
       if (Number.isFinite(route.mapIndex)) this.removeAudioMapping(route.mapIndex);
       break;
     case 'xy':
       if (route.key === 'translation_x') this.motionPadValues.translation_x = 0;
       if (route.key === 'translation_y') this.motionPadValues.translation_y = 0;
       this.queueLiveParam(route.key, 0);
       break;
     case 'morph': {
       const slot = this.morphSlots.find((item) => item.id === route.slotId);
       if (slot) {
         slot.on = false;
         this.applyPromptMorphing();
       }
 ...
 async refreshFrames(limit = 48) {
   ...
   const previousSelectedSrc = this.frameSrcKey(this.selectedFrameThumb ? (this.selectedFrameThumb.src || this.selectedFrameThumb.url || this.selectedFrameThumb.path || '') : '');
    const safeLimit = Math.max(1, Math.min(256, Number(limit) || 48));
    const res = await fetch(`/api/frames?limit=${safeLimit}`, { cache: "no-store" });
     if (!res.ok) {
...
onAudioUpload(evt) {
 ...
 applySequencerTimeline(tl, { name } = {}) {
   ...
   this.sequencer.tracks = Array.isArray(tl.tracks)
     ...
     : [];
   if (name) this.sequencerSaveName = name;
 ...
 async loadSequencerTimeline() {
   ...
   try {
     ...
     this.applySequencerTimeline(j.timeline, { name });
     this.sequencerStatus = "Loaded " + name;
   } catch (e) {
     this.sequencerStatus = String(e.message || e);
   }
 },
 deforumSchedulePair(startVal, endVal, endFrame) {
   const start = Number(startVal) || 0;
   const end = Number(endVal) || 0;
   const frame = Math.max(1, Number(endFrame) || 1);
   return `0: (${start}), ${frame}: (${end})`;
 },
 buildDeforumSettingsForStartupClip(presetName, durationSec, fps) {
   const settings = JSON.parse(JSON.stringify(this.activeDeforumSettings()));
   const safeFps = Math.max(1, Number(fps) || 24);
   const safeDuration = Math.max(0.5, Number(durationSec) || 8);
   const maxFrames = Math.max(1, Math.round(safeDuration * safeFps));
   const endFrame = Math.max(1, maxFrames - 1);
   settings.max_frames = maxFrames;
   settings.fps = safeFps;
   const preset = this.motionPresets[presetName] || this.motionPresets.Orbit;
   const scheduleMap = {
   ...
   Object.entries(scheduleMap).forEach(([presetKey, deforumKey]) => {
     if (preset[presetKey] == null) return;
     settings[deforumKey] = this.deforumSchedulePair(0, preset[presetKey], endFrame);
   });
   return settings;
 },
 buildSequencerTracksFromMotionPreset(presetName, durationSec) {
   const preset = this.motionPresets[presetName] || this.motionPresets.Orbit;
   const duration = Math.max(0.5, Number(durationSec) || 8);
   const params = ["translation_x", "translation_y", "translation_z", "rotation_y", "rotation_z"];
   const tracks = [];
   params.forEach((param) => {
     if (preset[param] == null) return;
     const endVal = Number(preset[param]) || 0;
     tracks.push({
       id: `tr-boot-${param}`,
       param,
       keyframes: [
         { t: 0, v: 0 },
         { t: duration, v: endVal, easing: "easeInOut" },
       ],
     });
   });
   return tracks;
 },
 applyStartupSequencerTimeline(presetName, durationSec, fps) {
   const safeDuration = Math.max(0.5, Number(durationSec) || 8);
   const safeFps = Math.max(1, Number(fps) || 24);
   const timeline = {
     version: 1,
     ...
     fps: safeFps,
     loop: true,
     markers: [{ t: 0, name: "Intro", action: "jump", target: "" }],
     tracks: this.buildSequencerTracksFromMotionPreset(presetName, safeDuration),
   };
   this.applySequencerTimeline(timeline, { name: this.sequencerSaveName || "default_clip" });
   this.sequencerPlayhead = 0;
 },
 async bootstrapStartupSequencerClip() {
   if (this.startupSequencerBootstrapped || this.startupSequencerBootstrapping) return;
   if (typeof fetch !== "function") return;
   this.startupSequencerBootstrapping = true;
   const clipName = (this.sequencerSaveName || "default_clip").trim().replace(/[^a-zA-Z0-9_-]/g, "") || "default_clip";
   const durationSec = 8;
   const fps = Math.max(1, Number(this.sequencer.fps || this.deforumSettings.fps || 24) || 24);
   const presetName = this.motionSelectedPreset || "Orbit";
   try {
     await this.refreshSequencerList();
     if (this.sequencerList.includes(clipName)) {
       this.sequencerLoadPick = clipName;
       await this.loadSequencerTimeline();
       this.performance.status = `Loaded sequence "${clipName}"`;
       return;
     }
     this.performance.status = "Rendering 8s Deforum clip for sequencer…";
     this.sequencerStatus = "Rendering startup clip…";
     const settings = this.buildDeforumSettingsForStartupClip(presetName, durationSec, fps);
     const res = await fetch("/api/deforum/clip", {
       method: "POST",
       headers: { "Content-Type": "application/json" },
       body: JSON.stringify({ settings, durationSec, fps }),
     });
     const data = await res.json();
     if (!res.ok || data.error) throw new Error(data.error || "Deforum clip failed");
     const expectedFrames = Math.max(1, Math.round(durationSec * fps));
     const deadline = Date.now() + 180000;
     while (Date.now() < deadline) {
       await this.refreshFrames(Math.min(256, expectedFrames + 16));
       if (this.thumbs.length >= Math.min(expectedFrames, 12)) break;
       await new Promise((resolve) => setTimeout(resolve, 1200));
     }
     this.applyStartupSequencerTimeline(presetName, durationSec, fps);
     this.sequencerSaveName = clipName;
     await this.saveSequencerTimeline();
     this.sequencerLoadPick = clipName;
     this.performance.status = `Startup sequence "${clipName}" ready`;
     this.sequencerStatus = `Loaded ${clipName} (${this.thumbs.length} frames)`;
     if (data.path) {
     ...
     this.applyStartupSequencerTimeline(presetName, durationSec, fps);
     this.sequencerStatus = `Startup clip unavailable: ${String(err.message || err)}`;
     this.performance.status = "Using motion preset timeline (clip render skipped)";
     if (!this.deforumPlaying) this.schedulePreviewFrame();
   } finally {
     this.startupSequencerBootstrapping = false;
     this.startupSequencerBootstrapped = true;
   }
 },
